...
namespace astrum
{
  ...
  const auto rock_instance_size = sizeof(ludo::mat4) + sizeof(uint32_t) + 12; // align 16
  ...
  void add_rocks(ludo::instance& inst, uint32_t celestial_body_index)
  {
    ...
    auto render_program = ludo::add(
      ...
      ludo::render_program
      {
        ...
        .instance_size = rock_instance_size,
        .push_on_bind = false
      },
      "rocks"
    );
  ...
  {
    auto grid = ludo::first<ludo::grid3>(inst, "rocks");
    auto& render_meshes = ludo::data<ludo::render_mesh>(inst, "rocks");
    auto rendering_context = ludo::first<ludo::rendering_context>(inst);
    auto render_program = ludo::first<ludo::render_program>(inst, "rocks");
      ...
      else if (lod_index == 0 && chunk.trees_loaded)
      {
        for (auto& tree_render_mesh_id : chunk.tree_render_mesh_ids)
        {
          if (tree_render_mesh_id)
          {
            ...
            ludo::remove(inst, render_mesh, "rocks");
          }
        }
  ...
  )
  {
    ...
    for (auto tree_type = 0; tree_type < trees.size(); tree_type++)
    {
      ...
      auto render_mesh = ludo::add(inst, ludo::render_mesh(), "rocks");
